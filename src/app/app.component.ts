 ...
 */
  subscribeService(): void{
    ...
    this.subscription = this.sharedService.weatherData$.asObservable().subscribe((data: IWeatherData) => {
                          ...
                          if (data && index > -1){
                            const tempWeatherList = [...this.weatherList];
                            tempWeatherList[index] = data;
                            this.weatherList = tempWeatherList;
                          }
 ...
 */
  addCity(cityEl: HTMLInputElement): void{
   ...
   */
  headerEvent(headerEventData: any): void{
   ...
   */
  deleteCity(selectedCity: any): void{
  ...
  getNextHourWeatherData(list: IForecastDtl[]): IForecastDtl[] {
    const selectedForecastData: IForecastDtl[] = [];
  ...
  showWeatherForecast(forecastData: any): void{
   ...
   */
  showToastMessage(type: string, message: string,  isShown: boolean= true): void{
